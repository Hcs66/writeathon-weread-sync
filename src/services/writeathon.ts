...
// Writeathon API基础URL
const API_BASE_URL = 'https://api.writeathon.cn';
...
export const writeathonService = {
  ...
  // 获取用户信息
  async getUserInfo(settings: WriteathonSettings): Promise<{username: string} | null> {
    try {
      const response = await fetch(`${API_BASE_URL}/v1/me`, {
        method: 'GET',
        headers: {
  ...
  async createCard(
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/v1/users/${settings.userId}/cards`, {
        method: 'POST',
        headers: {
  ...
  // 获取卡片内容
  async getCard(settings: WriteathonSettings, title: string): Promise<any | null> {
    try {
      const response = await fetch(`${API_BASE_URL}/v1/users/${settings.userId}/cards/get`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-writeathon-token': settings.apiToken
